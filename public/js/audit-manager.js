...
const AuditManager = (() => {
  ...
  async function load() {
    ...
    tbody.innerHTML = '<tr><td colspan="6" class="loading-placeholder">Loading audit logs…</td></tr>';
    try {
      const res = await DB.list(SD.COL.AUDIT_LOGS, [SD.Q.orderDesc('timestamp')], 300);
      allLogs   = res.documents.map(d=>({id:d.$id,...d}));
      renderTable(allLogs);
    } catch(e) { tbody.innerHTML=`<tr><td colspan="6" style="color:#dc3545">${e.message}</td></tr>`; }
  }

  function renderTable(logs) {
    const tbody = document.getElementById('auditBody'); if(!tbody) return;
    if (!logs.length) { tbody.innerHTML='<tr><td colspan="6" class="loading-placeholder">No logs found.</td></tr>'; return; }
    tbody.innerHTML = logs.map(l => {
      const ts = l.timestamp ? new Date(l.timestamp).toLocaleString('en-NG') : '--';
      ...
      return `<tr>
        ...
        <td>${l.action||'—'}</td>
        <td>${l.userId||'—'}</td>
        <td style="max-width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:.8rem">${JSON.stringify(meta).substring(0,80)}</td>
  ...
  function filter() {
    ...
    renderTable(allLogs.filter(l => {
      if (type!=='all'     && !l.action?.toLowerCase().includes(type.toLowerCase())) return false;
  ...
  async function exportLogs() {
    ...
    const rows = allLogs.map(l => {
      ...
      return {
        ...
        Action:    l.action||'', User: l.userId||'',
        Severity:  l.severity||'', Details: JSON.stringify(meta).substring(0,200),
      };
    });
