/**
 * SOFTLY DIGITAL V3 — icons.js
 * Centralized SVG icon library. Replaces all emoji usage.
 * Usage: Icons.get('dashboard', { size: 20, className: 'icon' })
 *        Icons.get('dashboard') returns raw SVG markup string.
 */
const Icons = (() => {
  'use strict';

  // Each entry is a Feather/Lucide-style 24x24 stroke icon, currentColor based.
  const PATHS = {
    ...
    'shield':       '<path d="M12 2 4 5v6c0 5.25 3.5 9.5 8 11 4.5-1.5 8-5.75 8-11V5l-8-3z"/>',
    'shield-check': '<path d="M12 2 4 5v6c0 5.25 3.5 9.5 8 11 4.5-1.5 8-5.75 8-11V5l-8-3z"/><path d="m9 12 2 2 4-4"/>',
    ...
    'alert-triangle':'<path d="m10.29 3.86-8.18 14.14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.89-3l-8.18-14.14a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/>',
    ...
    'shield-off':   '<path d="M19.69 14a6.9 6.9 0 0 0 .31-2V5l-8-3-3.16 1.18"/><path d="M4.73 4.73 4 5v7c0 6 8 10 8 10a20.3 20.3 0 0 0 5.62-4.38"/><line x1="1" y1="1" x2="23" y2="23"/>',
    ...
    'qr-code':      '<rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><line x1="14" y1="14" x2="14" y2="14.01"/><line x1="21" y1="14" x2="21" y2="14.01"/><line x1="14" y1="21" x2="14" y2="21.01"/><line x1="21" y1="21" x2="21" y2="21.01"/><line x1="17.5" y1="17.5" x2="17.5" y2="17.5"/>',
  ...
  function raw(name) {
    return PATHS[name] || PATHS['help-circle'];
  }

  /**
   * Returns a complete <svg> markup string.
   * opts: { size (default 20), className, strokeWidth (default 2), title }
   */
  function get(name, opts = {}) {
    const size   = opts.size || 20;
    const cls    = opts.className ? ` class="${opts.className}"` : ' class="sd-icon"';
    const sw     = opts.strokeWidth || 2;
    const titleEl= opts.title ? `<title>${opts.title}</title>` : '';
    return `<svg${cls} width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${sw}" stroke-linecap="round" stroke-linejoin="round" aria-hidden="${opts.title ? 'false' : 'true'}">${titleEl}${raw(name)}</svg>`;
  }

  /** Replace any element with data-icon="name" attribute with inline SVG. Call on DOMContentLoaded and after dynamic renders. */
  function hydrate(root) {
    ...
    scope.querySelectorAll('[data-icon]').forEach(el => {
      ...
      if (!el.dataset.iconHydrated) {
        el.innerHTML = get(name, { size: parseInt(size) });
  ...
  return { get, raw, hydrate, names: () => Object.keys(PATHS) };
})();
